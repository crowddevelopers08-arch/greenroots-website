...
import { PRODS, getCategorySlug, getEdibleBrandHref, type Product } from "@/lib/store-data";
...
export function EdibleBrandPage({ brandProduct, images }: Props) {
  ...
  return (
    <>
      ...
      >
        <div className="w-full">
          ...
          <div className="mb-6 flex flex-wrap items-center gap-2 text-[length:var(--fs-caption)] text-[#3d5843]">
            ...
            <Link
              href={`/?category=${getCategorySlug("Edible")}`}
          ...
          <div className="mb-6 overflow-x-auto pb-1">
            <div className="flex min-w-max gap-2">
              {PRODS["Edible"].map((brand) => {
                const isActive = brand.id === brandProduct.id;
                return (
                  <Link
                    key={brand.id}
                    href={getEdibleBrandHref(brand.name)}
      ...
      <ImageLightbox
        src={lightboxImg ?? ""}
        alt={`${brandProduct.name} catalogue image`}
